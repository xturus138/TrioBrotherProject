...
import { VideoIcon, HeartIcon, ShareIcon, MoreVerticalIcon, EditIcon, TrashIcon } from "lucide-react"
...
export function AssetCard({ asset, folders, currentUserId, onShare }: AssetCardProps) {
    ...
    <>
      <Card className="overflow-hidden hover:shadow-md transition-shadow">
        ...
        <CardContent className="p-4">
          {asset.caption && <p className="text-sm text-gray-700 mb-2 line-clamp-2">{asset.caption}</p>}
          <div className="flex items-center justify-between text-xs text-gray-500">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <HeartIcon className={`w-4 h-4 ${isHearted ? "fill-current" : ""}`} />
                <span>{hearts}</span>
              </button>
              <button onClick={() => onShare?.(asset)} className="hover:text-indigo-600 transition-colors">
